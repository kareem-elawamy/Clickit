...
// 🚀 بداية أكواد التحسين: تجميع الكاش لمدة دقيقة لتقليل الاستعلامات
$sidebarOrderCounts = Cache::remember('sidebar_order_counts', 60, function() {
return \App\Models\Order::select('order_status', DB::raw('count(*) as total'))
->groupBy('order_status')
->pluck('total', 'order_status')
->toArray();
});
$sidebarTotalOrders = array_sum($sidebarOrderCounts);

$sidebarRefundCounts = Cache::remember('sidebar_refund_counts', 60, function() {
return \App\Models\RefundRequest::select('status', DB::raw('count(*) as total'))
->groupBy('status')
->pluck('total', 'status')
->toArray();
});

$sidebarContactCount = Cache::remember('sidebar_contact_count', 60, function() {
return \App\Models\Contact::where('seen', 0)->count();
});

$sidebarTicketCount = Cache::remember('sidebar_ticket_count', 60, function() {
return \App\Models\SupportTicket::where('status', 'open')->count();
});
// 🚀 نهاية أكواد التحسين
@endphp
                ...
                <a class="nav-link {{ Request::is('admin/contact*') ? 'active' : '' }}"
                    ...
                    <span class="aside-mini-hidden-element text-truncate">
                        <span class="position-relative">
                            {{ translate('messages') }}
                            @php($message = $sidebarContactCount)
                            @if($message != 0)
                            <span
